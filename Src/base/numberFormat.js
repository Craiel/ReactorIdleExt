var names = {
    ...
    nfPlus = function(n) {
        var o = ""
        return n > 0 && (o = "+"), o + numberFormat.format(n)
    },
    numberFormat = {
        format: function(n) {
            if (void 0 == n) return "???"
            ...
            var o = n[0],
                i = 0 > o ? 2 : 1,
                l = 3 * Math.floor((Number(o).toFixed(0).length - i) / 3),
                e = l + (n[1] ? Number(n[1]) : 0),
                r = e % 3
            return o *= Math.pow(10, r - l), e -= r, Math.round(100 * o) / 100 + (names[e] ? names[e] : "e" + e)
        },
        test: function() {
            for (var n in nums) {
                var o = nf(n)
